...
class _MessagesProxy:
    ...
    def create(self, **kwargs: Any) -> Any:
        """Intercept a messages.create() request."""
        # 1. Rate limit
        ...
        if self._budget_manager is not None:
            estimated = CostTracker.calculate_cost(model, 500, 500)
        ...
        input_tokens, output_tokens, cache_creation_tokens, cache_read_tokens = (
            AnthropicProvider.extract_usage_detailed(response)
        )
...
class _AnthropicClientProxy:
    """Top-level proxy that replaces ``client.messages``."""

    def __init__(self, original_client: Any, messages_proxy: _MessagesProxy) -> None:
...
class AnthropicProvider(BaseProvider):
    """Wraps an Anthropic client to add cost tracking, budgets, PII scanning."""
    ...
    def wrap(self, client: Any) -> Any:
        """Return a proxy that instruments ``client.messages.create()``."""
    ...
    def extract_usage_detailed(response: Any) -> tuple[int, int, int, int]:
        ...
        Anthropic reports cache tokens separately from input_tokens — no subtraction needed.
        """
        usage = getattr(response, "usage", None)
        if usage is None:
            return (0, 0, 0, 0)
